...
class ModelGPTPredictor:
    ...
    def load_pred_ranking_2(self, model="mpnet", type="template", thres="0.5", transform_k=1, m=0.9, use_model_avg_rank=False):
        if use_model_avg_rank:
            path = "ptm_stats_test/pred_stats_on_target_task/pred_model_rank/model_class_rank_mean.pkl"
        else:
            path = f"ptm_stats_test/pred_stats_on_target_task/pred_model_rank/{model}_{type}_{thres}_{transform_k}_{m}.pkl"
        f = open(path, 'rb')
    ...
    def predict_model_rank(self, test_dataset:set) -> Tuple[pd.DataFrame, dict]:
        ...
        for d in np.unique(df.dataset):
            ...
            df_dataset[f"norm_{self.pred_target}"] = (df_dataset[self.pred_target] - dataset_mean).values
