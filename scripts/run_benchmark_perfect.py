...
# -*- coding: utf-8 -*-
"""
VEGA vs LogME Benchmark Script (Perfect Version with Corrected Softmax Temperature)
Compare VEGA and LogME methods on model selection tasks

此版本使用 VEGAPerfectScorer，包含以下关键修复:
...
# Import PERFECT VEGA implementation
from methods.baseline.vega_perfect import VEGAPerfectScorer

# ============================================================================
# Configuration
# ============================================================================

CACHE_DIR = project_root / "cache_perfect"
...
def get_cache_key(model_name, dataset_name, method, pca_dim=64, temperature=0.01):
    key_str = "%s_%s_%s_perfect_pca%d_t%.4f" % (model_name, dataset_name, method, pca_dim, temperature)
    return hashlib.md5(key_str.encode()).hexdigest()


def get_cache_path(model_name, dataset_name, method, pca_dim=64, temperature=0.01):
    cache_key = get_cache_key(model_name, dataset_name, method, pca_dim, temperature)
    return CACHE_DIR / ("%s.pkl" % cache_key)


def save_cache(model_name, dataset_name, method, data, pca_dim=64, temperature=0.01):
    if not ENABLE_CACHE:
        return
    cache_path = get_cache_path(model_name, dataset_name, method, pca_dim, temperature)
    cache_data = {
        'model': model_name,
...
def load_cache(model_name, dataset_name, method, pca_dim=64, temperature=0.01):
    if not ENABLE_CACHE:
        return None
    cache_path = get_cache_path(model_name, dataset_name, method, pca_dim, temperature)
...
# ============================================================================
# VEGA Score Computation (Perfect Version with Corrected Softmax Temperature)
# ============================================================================
...
def compute_vega_score_perfect(
    ...
    """
    Compute VEGA score using PERFECT VEGAPerfectScorer class
    
    关键修复:
    1. Softmax 温度从 0.05 改为 0.01
       - CLIP 的 logit_scale ≈ 100 对应 t=0.01
       - 原 t=0.05 太"热"，导致 s_n ≈ 1/K
       - 现在 s_n = mean(softmax(cos/0.01) at pseudo_labels)
    
    2. 保留 Bhattacharyya 系数 (exp(-D_B))
       - 正确度量相似度
       - Pearson 相关为正（好模型）
    ...
    3. 保留 PCA + Shrinkage
       ...
       - 保证协方差正定
    """
    print_detail("Computing VEGA score (完美版本 - VEGAPerfectScorer):", indent=4)
    ...
    print_detail("- Softmax temperature: %.4f (修复: 0.05 -> 0.01)" % temperature, indent=6)
    
    start_time = time.time()
    
    try:
        # Use PERFECT VEGAPerfectScorer class
        ...
        print_detail("  - Node similarity: %.4f (Softmax, t=%.4f)" % (node_sim, temperature), indent=6)
        print_detail("  - Edge similarity: %.4f (Bhattacharyya 系数)" % edge_sim, indent=6)
...
def run_single_dataset_benchmark(
    ...
    for idx, model_name in enumerate(model_list):
        ...
        cached_vega = load_cache(model_name, dataset_name, 'vega', pca_dim, temperature)
        ...
        # VEGA (完美版本 - 修复 Softmax 温度)
        if cached_vega is not None:
            vega_score = cached_vega.get('score')
            vega_detail = cached_vega
        else:
            vega_score, vega_detail = compute_vega_score_perfect(
                img_feat, text_feat, logits, model_name,
                pca_dim=pca_dim,
                temperature=temperature
            )
            if vega_score is not None:
                save_cache(model_name, dataset_name, 'vega', vega_detail, pca_dim, temperature)
        if vega_score is not None:
    ...
    print("\n" + "-" * 70)
    print("VEGA Detailed Analysis (Softmax 温度修复版):")
    print("-" * 70)
...
def print_final_results(all_results, pca_dim=64, temperature=0.01):
    """Print final summary results"""
    print("\n" + "=" * 70)
    print("Summary Results (完美版本 - VEGAPerfectScorer)")
    print("=" * 70)
    print("PCA dim: %d" % pca_dim)
    print("Softmax temperature: %.4f (修复: 0.05 -> 0.01)" % temperature)
    print("\n关键修复:")
    print("  1. Softmax 温度: t=0.01 (匹配 CLIP logit_scale)")
    print("     - s_n = mean(softmax(cos/0.01) at pseudo_labels)")
    print("     - 解决 s_n ≈ 1/K 问题")
    print("  2. E^V = exp(-D_B) Bhattacharyya 系数")
    print("     - 正确度量相似度")
    print("  3. s = s_n + s_e 自然融合")
    ...
    if valid_vega:
        ...
        print("\nVEGA-Perfect (on %d datasets):" % len(valid_vega))
...
def main():
    ...
    print("=" * 70)
    print("VEGA vs LogME Benchmark (完美版本 - VEGAPerfectScorer)")
    print("=" * 70)
    print("Data directory: %s" % data_dir)
    print("Cache directory: %s" % CACHE_DIR)
    print("PCA dimension: %d" % pca_dim)
    print("Softmax temperature: %.4f (修复: 0.05 -> 0.01)" % temperature)
    print("\n此版本使用 VEGAPerfectScorer，包含以下修复:")
    print("  1. 【修复 Softmax 温度】t=0.01 (匹配 CLIP logit_scale)")
    print("     - 原 t=0.05 太热，导致 s_n ≈ 1/K")
    print("     - CLIP logit_scale ≈ 100 对应 t=0.01")
    print("     - s_n = mean(softmax(cos/0.01) at pseudo_labels)")
    print("  2. 【保留 Bhattacharyya 系数】exp(-D_B)")
    print("     - 正确度量相似度 (非距离)")
    print("     - Pearson 相关为正（好模型）")
    print("  3. 【保留 PCA + Shrinkage】")
    print("     - 解决维度诅咒")
    print("     - 保证协方差正定")
    print("  4. 【自然融合】s = s_n + s_e")
    ...
    print_final_results(all_results, pca_dim, temperature)
    
    result_file = CACHE_DIR / "benchmark_results_perfect.json"
