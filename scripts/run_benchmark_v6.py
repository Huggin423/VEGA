...
- 2026-03-13: v6 切换文本特征源为 caption_text_feat
"""
...
from methods.baseline.vega_v6 import VEGAOptimizedScorer, compute_tau_at_k

# ============================================================================
# Configuration
# ============================================================================

CACHE_DIR = project_root / "cache_v6"
CACHE_DIR.mkdir(exist_ok=True)
ENABLE_CACHE = False
ALLOW_FALLBACK_TO_CLASS_TEXT_FEAT = False
...
def get_cache_key(model_name, dataset_name, method, pca_dim=64, node_weight=1.0, edge_weight=1.0):
    key_str = "%s_%s_%s_optimized_v6_caption_text_feat_pca%d_n%.1f_e%.1f" % (model_name, dataset_name, method, pca_dim, node_weight, edge_weight)
...
def load_text_features(data_dir, model_name, dataset_name, verbose=False):
    """Load model text features (caption_text_feat as primary source)."""
    search_paths = [
        os.path.join(data_dir, 'ptm_stats/stats_on_hist_task/caption_text_feat', '%s.pkl' % model_name),
    ]
    if ALLOW_FALLBACK_TO_CLASS_TEXT_FEAT:
        search_paths.append(os.path.join(data_dir, 'ptm_stats/class_text_feat', '%s.pkl' % model_name))
    feat_path = None
...
# ============================================================================
# VEGA Score Computation (Optimized V6 Version)
# ============================================================================
...
def main():
    ...
    print("=" * 70)
    print("VEGA vs LogME Benchmark (优化版本 V6 - VEGAOptimizedScorer)")
    print("=" * 70)
    ...
    print("Text feature source: ptm_stats/stats_on_hist_task/caption_text_feat")
    print("Fallback to class_text_feat: %s" % ALLOW_FALLBACK_TO_CLASS_TEXT_FEAT)
    ...
    result_file = CACHE_DIR / "benchmark_results_v6_caption_text_feat.json"
