...
# -*- coding: utf-8 -*-
"""
VEGA vs LogME Benchmark Script (优化版本)
Compare VEGA and LogME methods on model selection tasks

此版本使用 VEGAOptimizedScorer，包含以下关键优化:
...
Changelog:
- 2026-03-11: 创建优化版本，使用 VEGAOptimizedScorer
"""
...
from methods.baseline.vega_optimized import VEGAOptimizedScorer

# ============================================================================
# Configuration
# ============================================================================

CACHE_DIR = project_root / "cache_optimized"
...
def get_cache_key(model_name, dataset_name, method):
    key_str = "%s_%s_%s_optimized" % (model_name, dataset_name, method)
...
# ============================================================================
# VEGA Score Computation (Optimized Version)
# ============================================================================

def compute_vega_score_optimized(img_features, text_features, logits, model_name=""):
    """
    Compute VEGA score using OPTIMIZED VEGAOptimizedScorer class
    
    四项关键优化:
    1. 修复 Node Similarity 的范围溢出 Bug
    ...
    2. 对角协方差近似 (Diagonal Covariance Approximation)
       - VLM 特征维度高达 512/768，小样本下全协方差矩阵不可逆
    ...
    try:
        # Use OPTIMIZED VEGAOptimizedScorer class
        vega = VEGAOptimizedScorer(
            temperature=0.05,           # 论文默认值
            min_samples_per_class=2,    # 需要至少2个样本计算方差
            regularization=1e-6         # 对角方差正则化
        )
        ...
        print_detail("  - Diagonal covariance: True", indent=6)
        print_detail("  - Vectorized computation: True", indent=6)
        print_detail("  - Node similarity fix: True (除以 N_total)", indent=6)
...
def run_single_dataset_benchmark(data_dir, dataset_name, model_list, verbose=True):
    ...
    for idx, model_name in enumerate(model_list):
        ...
        if cached_vega is not None:
            ...
            vega_score, vega_details = compute_vega_score_optimized(img_feat, text_feat, logits, model_name)
...
def print_final_results(all_results):
    ...
    if valid_vega:
        ...
        print("  优化项:")
        print("    - Node Similarity: 除以 N_total 而非 K (修复溢出 Bug)")
        print("    - 对角协方差近似: 避免高维矩阵不可逆问题")
        print("    - 向量化 Bhattacharyya 距离: 消除双重 for 循环")
        print("    - Edge Similarity: 保留正确逻辑，NaN 处理")
    else:
...
def main():
    ...
    print("\n此版本使用 VEGAOptimizedScorer，包含以下优化:")
    print("  1. 修复 Node Similarity 的范围溢出 Bug")
    print("     - 除以 N_total 而非 K")
    print("     - 移除 np.clip 截断")
    print("  2. 对角协方差近似 (Diagonal Covariance Approximation)")
    print("     - 避免高维协方差矩阵不可逆问题")
    print("     - 添加正则化项防止除零")
    print("  3. 向量化 Bhattacharyya 距离计算")
    print("     - 利用 PyTorch 广播机制")
    print("     - 消除双重 for 循环")
    print("  4. 保留 Edge Similarity 正确逻辑")
    print("     - 使用上三角元素计算 Pearson 相关系数")
    print("     - NaN 异常处理")
    ...
    result_file = CACHE_DIR / "benchmark_results_optimized.json"
