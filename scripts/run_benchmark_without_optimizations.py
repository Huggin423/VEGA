...
from methods.baseline.vega_without_optimization import VEGAOriginalScorer

# ============================================================================
# Configuration
# ============================================================================

CACHE_DIR = project_root / "cache_original"
...
def get_cache_key(model_name, dataset_name, method):
    key_str = "%s_%s_%s_original" % (model_name, dataset_name, method)
...
# ============================================================================
# VEGA Score Computation (Original Version - No Optimizations)
# ============================================================================

def compute_vega_score_original(img_features, text_features, logits, model_name=""):
    """
    Compute VEGA score using ORIGINAL algorithm (无优化版本)
    ...
    论文公式:
    - 节点相似度: s_n = (1/K) Σ_k sim(n^T_k, n^V_k) · N_k
    - 边相似度: s_e = (corr(E^T, E^V) + 1)/2
    - 最终分数: s = s_n + s_e
    """
    print_detail("Computing VEGA score (ORIGINAL version - no optimizations):", indent=4)
    ...
    try:
        # Use ORIGINAL VEGAScorer class (无优化)
        vega = VEGAOriginalScorer(
        ...
        print_detail("  - Use PCA: False", indent=6)
        print_detail("  - Full covariance: True", indent=6)
        print_detail("  - Vectorized computation: False", indent=6)
...
def run_single_dataset_benchmark(data_dir, dataset_name, model_list, verbose=True):
    ...
    for idx, model_name in enumerate(model_list):
        ...
        if cached_vega is not None:
            ...
            vega_score, vega_details = compute_vega_score_original(img_feat, text_feat, logits, model_name)
...
def print_final_results(all_results):
    ...
    if valid_vega:
        ...
        print("\nVEGA-Original (on %d datasets):" % len(valid_vega))
        ...
        print("  (无PCA, 完整协方差矩阵, 双重循环计算)")
    else:
        print("\nVEGA-Original: No valid results")
...
def main():
    ...
    print("=" * 70)
    print("VEGA vs LogME Benchmark (原始无优化版本)")
    print("=" * 70)
    ...
    print("\n注意: 此版本使用原始VEGA算法")
    print("  - 无PCA降维/白化")
    print("  - 使用完整协方差矩阵（非对角近似）")
    print("  - 使用双重for循环计算Bhattacharyya距离（非向量化）")
    print("  - 严格遵循论文公式计算节点相似度")
    print("  - 计算速度较慢，但结果更接近论文原始算法")
    ...
    result_file = CACHE_DIR / "benchmark_results_original.json"
