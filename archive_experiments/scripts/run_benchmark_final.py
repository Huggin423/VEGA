...
# -*- coding: utf-8 -*-
"""
VEGA vs LogME Benchmark Script (Final Version with PCA Reduction and Weighted Fusion)
Compare VEGA and LogME methods on model selection tasks

此版本使用 VEGAFinalScorer，包含以下关键改进:
1. PCA 降维 - 解决高维特征的维度诅咒问题
   - 将 D=512/768/1024 降至 pca_dim=64
   - 解决 O(D³) 计算瓶颈
   - 恢复 Bhattacharyya 距离的几何意义

2. 加权融合 - 解决 s_n 和 s_e 尺度不匹配问题
   - score = w_n * s_n + w_e * s_e
...
# Import FINAL VEGA implementation
from methods.baseline.vega_final import VEGAFinalScorer

# ============================================================================
# Configuration
# ============================================================================

CACHE_DIR = project_root / "cache_final"
...
def get_cache_key(model_name, dataset_name, method, pca_dim=64, node_weight=1.0, edge_weight=0.0):
    key_str = "%s_%s_%s_final_pca%d_n%.1f_e%.1f" % (model_name, dataset_name, method, pca_dim, node_weight, edge_weight)
...
# ============================================================================
# VEGA Score Computation (Final Version with PCA Reduction)
# ============================================================================
...
def compute_vega_score_final(
    ...
    """
    Compute VEGA score using FINAL VEGAFinalScorer class
    ...
    2. 加权融合
       - score = w_n * s_n + w_e * s_e
       - 解决 s_n (~0.01) 和 s_e (~0.5) 尺度不匹配问题
    
    3. Shrinkage 正则化 + 向量化计算
       - 在低维空间应用完整协方差 + Shrinkage
       - 批量矩阵运算
    """
    print_detail("Computing VEGA score (最终版本 - VEGAFinalScorer):", indent=4)
    ...
    try:
        # Use FINAL VEGAFinalScorer class
        ...
        print_detail("  - Node similarity: %.4f (weight=%.2f)" % (node_sim, node_weight), indent=6)
        print_detail("  - Edge similarity: %.4f (weight=%.2f)" % (edge_sim, edge_weight), indent=6)
...
def print_final_results(all_results, pca_dim=64, node_weight=1.0, edge_weight=0.0):
    ...
    print("\n" + "=" * 70)
    print("Summary Results (最终版本 - VEGAFinalScorer)")
    print("=" * 70)
    ...
    if valid_vega:
        ...
        print("\nVEGA-Final (on %d datasets):" % len(valid_vega))
        ...
        print("  改进项:")
        print("    - PCA 降维: 解决维度诅咒，恢复几何意义")
        print("    - 加权融合: 解决尺度不匹配")
        print("    - Shrinkage + 向量化: 保证稳定性")
    else:
        print("\nVEGA-Final: No valid results")
...
def main():
    ...
    # Configuration
    pca_dim = 64
    node_weight = 1.0  # Test with node similarity only
    edge_weight = 0.0
    ...
    print("=" * 70)
    print("VEGA vs LogME Benchmark (最终版本 - VEGAFinalScorer)")
    print("=" * 70)
    ...
    print("\n此版本使用 VEGAFinalScorer，包含以下改进:")
    print("  1. PCA 降维 (%d 维)" % pca_dim)
    print("     - 解决 O(D³) 计算瓶颈")
    print("     - 恢复 Bhattacharyya 距离的几何意义")
    print("  2. 加权融合")
    print("     - score = %.1f * s_n + %.1f * s_e" % (node_weight, edge_weight))
    print("     - 解决 s_n (~0.01) 和 s_e (~0.5) 尺度不匹配")
    print("  3. Shrinkage + 向量化")
    print("     - 在低维空间应用完整协方差")
    ...
    result_file = CACHE_DIR / "benchmark_results_final.json"
