...
# -*- coding: utf-8 -*-
"""
VEGA vs LogME Benchmark Script (Robust Version with Full Covariance + Shrinkage)
Compare VEGA and LogME methods on model selection tasks

此版本使用 VEGARobustScorer，包含以下关键改进:
...
Changelog:
- 2026-03-11: 创建鲁棒版本，使用 VEGARobustScorer (完整协方差 + Shrinkage)
"""
...
# Import ROBUST VEGA implementation
...
CACHE_DIR = project_root / "cache_robust"
...
def get_cache_key(model_name, dataset_name, method):
    key_str = "%s_%s_%s_robust" % (model_name, dataset_name, method)
...
# ============================================================================
# VEGA Score Computation (Robust Version with Full Covariance + Shrinkage)
# ============================================================================

def compute_vega_score_robust(img_features, text_features, logits, model_name=""):
    """
    Compute VEGA score using ROBUST VEGARobustScorer class
    
    四项关键改进:
    1. 完整协方差矩阵 + Shrinkage 正则化
       - 解决高维特征 (D=512/768) 下样本协方差矩阵 rank-deficient 问题
       - 公式: Σ_shrunk = (1-α)Σ + α·T, T = trace(Σ)/D · I
       - 保证协方差矩阵严格正定，可逆
    
    2. 向量化 Bhattacharyya 距离计算
       - 使用 PyTorch 批量矩阵运算
       - torch.linalg.inv() 批量求逆
       - torch.linalg.slogdet() 计算 log-determinant，避免溢出
       - 完全向量化，无 for 循环
    ...
    4. Edge Similarity
       ...
       - NaN 时回退到 0.5
    """
    print_detail("Computing VEGA score (鲁棒版本 - VEGARobustScorer):", indent=4)
    ...
    try:
        # Use ROBUST VEGARobustScorer class
        vega = VEGARobustScorer(
            temperature=0.05,           # 论文默认值
            min_samples_per_class=2,    # 需要至少2个样本计算协方差
            shrinkage_alpha=0.1         # Shrinkage 正则化参数
        )
        ...
        print_detail("  - Full covariance: True", indent=6)
        print_detail("  - Shrinkage alpha: 0.1", indent=6)
        print_detail("  - Vectorized computation: True", indent=6)
...
def run_single_dataset_benchmark(data_dir, dataset_name, model_list, verbose=True):
    ...
    vega_scores = {}
    vega_details = {}  # Store detailed results for analysis
    logme_scores = {}
    ground_truth = {}
    failed_models = []
    ...
    for idx, model_name in enumerate(model_list):
        ...
        # VEGA (鲁棒版本 - 完整协方差 + Shrinkage)
        if cached_vega is not None:
            vega_score = cached_vega.get('score')
            vega_detail = cached_vega
        else:
            vega_score, vega_detail = compute_vega_score_robust(img_feat, text_feat, logits, model_name)
            if vega_score is not None:
                save_cache(model_name, dataset_name, 'vega', vega_detail)
        if vega_score is not None:
        ...
        if logme_oracle_score is not None:
            ...
            logme_scores[model_name] = logme_pseudo_score
    ...
    print("\n" + "-" * 70)
    print("VEGA Detailed Analysis (Node vs Edge Similarity):")
    print("-" * 70)
...
def print_final_results(all_results):
    ...
    print("\n" + "=" * 70)
    print("Summary Results (鲁棒版本 - VEGARobustScorer)")
    print("=" * 70)
    ...
    if valid_vega:
        ...
        print("\nVEGA-Robust (on %d datasets):" % len(valid_vega))
        ...
        print("  改进项:")
        print("    - Full Covariance + Shrinkage: 解决高维特征奇异性问题")
        print("    - Vectorized Bhattacharyya: PyTorch 批量矩阵运算")
        print("    - Node Similarity: N_total 归一化")
        print("    - Edge Similarity: 上三角 Pearson 相关系数")
    else:
        print("\nVEGA-Robust: No valid results")
...
def main():
    ...
    print("=" * 70)
    print("VEGA vs LogME Benchmark (鲁棒版本 - VEGARobustScorer)")
    print("=" * 70)
    ...
    print("\n此版本使用 VEGARobustScorer，包含以下改进:")
    print("  1. 完整协方差矩阵 + Shrinkage 正则化")
    print("     - 解决高维特征 (D=512/768) 下样本协方差矩阵 rank-deficient 问题")
    print("     - 公式: Σ_shrunk = (1-α)Σ + α·T, T = trace(Σ)/D · I")
    print("     - 保证协方差矩阵严格正定，可逆")
    print("  2. 向量化 Bhattacharyya 距离计算")
    print("     - 使用 PyTorch 批量矩阵运算")
    print("     - torch.linalg.inv() 批量求逆")
    print("     - torch.linalg.slogdet() 计算 log-determinant")
    print("  3. 正确的 Node Similarity")
    print("     - s_n = Σ_k sim_k · N_k / N_total")
    print("     - 自然落在 [0, 1] 范围")
    print("  4. Edge Similarity")
    print("     - 上三角元素 Pearson 相关系数")
    print("     - NaN 时回退到 0.5")
    ...
    result_file = CACHE_DIR / "benchmark_results_robust.json"
    with open(result_file, 'w') as f:
        ...
        for r in all_results:
            sr = {
                ...
                'vega_details': r.get('vega_details', {}),
