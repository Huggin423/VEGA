...
# -*- coding: utf-8 -*-
"""
VEGA vs LogME Benchmark Script (Calibrated Version with Logits-based Node Similarity)
Compare VEGA and LogME methods on model selection tasks

此版本使用 VEGACalibratedScorer，包含以下关键改进:
...
# Import CALIBRATED VEGA implementation
from methods.baseline.vega_calibrated import VEGACalibratedScorer

# ============================================================================
# Configuration
# ============================================================================

CACHE_DIR = project_root / "cache_calibrated"
...
def get_cache_key(model_name, dataset_name, method, pca_dim=64, node_weight=1.0, edge_weight=0.0):
    key_str = "%s_%s_%s_calibrated_pca%d_n%.1f_e%.1f" % (model_name, dataset_name, method, pca_dim, node_weight, edge_weight)
...
# ============================================================================
# VEGA Score Computation (Calibrated Version with Logits-based Node Similarity)
# ============================================================================
...
def compute_vega_score_calibrated(
    ...
    """
    Compute VEGA score using CALIBRATED VEGACalibratedScorer class
    
    关键改进:
    1. Logits-based Node Similarity
    ...
    """
    print_detail("Computing VEGA score (校准版本 - VEGACalibratedScorer):", indent=4)
    ...
    try:
        # Use CALIBRATED VEGACalibratedScorer class
        ...
        print_detail("  - Node similarity: %.4f (Logits-based, weight=%.2f)" % (node_sim, node_weight), indent=6)
...
def run_single_dataset_benchmark(
    ...
    for idx, model_name in enumerate(model_list):
        ...
        # VEGA (校准版本 - Logits-based Node Similarity)
        if cached_vega is not None:
            vega_score = cached_vega.get('score')
            vega_detail = cached_vega
        else:
            vega_score, vega_detail = compute_vega_score_calibrated(
    ...
    print("\n" + "-" * 70)
    print("VEGA Detailed Analysis (Logits-based 校准版):")
    print("-" * 70)
...
def print_final_results(all_results, pca_dim=64, node_weight=1.0, edge_weight=0.0):
    ...
    print("\n" + "=" * 70)
    print("Summary Results (校准版本 - VEGACalibratedScorer)")
    print("=" * 70)
    print("PCA dim: %d" % pca_dim)
    print("Node weight: %.2f, Edge weight: %.2f" % (node_weight, edge_weight))
    print("\n关键改进:")
    print("  1. Logits-based Node Similarity:")
    print("     - s_n = mean(softmax(logits) at pseudo_labels)")
    print("     - 使用模型原生 logit_scale，尊重置信度校准")
    ...
    if valid_vega:
        ...
        print("\nVEGA-Calibrated (on %d datasets):" % len(valid_vega))
        ...
        print("\nVEGA-Calibrated: No valid results")
...
def main():
    ...
    print("=" * 70)
    print("VEGA vs LogME Benchmark (校准版本 - VEGACalibratedScorer)")
    print("=" * 70)
    ...
    print("\n此版本使用 VEGACalibratedScorer，包含以下改进:")
    print("  1. 【Logits-based Node Similarity】")
    print("     - 直接使用模型的 logits (已包含原生 logit_scale)")
    print("     - s_n = mean(softmax(logits) at pseudo_labels)")
    print("     - 尊重每个模型的置信度校准")
    print("  2. 【Edge Similarity 权重设为 0】")
    print("     - CNN 和 ViT 的结构流形拓扑不同")
    ...
    result_file = CACHE_DIR / "benchmark_results_calibrated.json"
