...
# -*- coding: utf-8 -*-
"""
VEGA vs LogME Benchmark Script (Ultimate Version with Corrected Formulation)
Compare VEGA and LogME methods on model selection tasks

此版本使用 VEGAUltimateScorer，包含以下关键修复:

1. 【修复 s_e 矛盾】距离 vs 相似度的悖论
   - 问题: E^T 是相似度 (高=好)，E^V 是距离 (低=好)
   - Pearson(smallity, distance) 对好模型为负！
   - 修复: E^V = exp(-D_B) 转换为 Bhattacharyya 系数 (相似度)
...
# Import ULTIMATE VEGA implementation
from methods.baseline.vega_ultimate import VEGAUltimateScorer

# ============================================================================
# Configuration
# ============================================================================

CACHE_DIR = project_root / "cache_ultimate"
...
def get_cache_key(model_name, dataset_name, method, pca_dim=64):
    key_str = "%s_%s_%s_ultimate_pca%d" % (model_name, dataset_name, method, pca_dim)
    return hashlib.md5(key_str.encode()).hexdigest()


def get_cache_path(model_name, dataset_name, method, pca_dim=64):
    cache_key = get_cache_key(model_name, dataset_name, method, pca_dim)
    return CACHE_DIR / ("%s.pkl" % cache_key)


def save_cache(model_name, dataset_name, method, data, pca_dim=64):
    if not ENABLE_CACHE:
        return
    cache_path = get_cache_path(model_name, dataset_name, method, pca_dim)
    cache_data = {
        'model': model_name,
...
def load_cache(model_name, dataset_name, method, pca_dim=64):
    if not ENABLE_CACHE:
        return None
    cache_path = get_cache_path(model_name, dataset_name, method, pca_dim)
...
# ============================================================================
# VEGA Score Computation (Ultimate Version)
# ============================================================================
...
def compute_vega_score_ultimate(
    ...
    """
    Compute VEGA score using ULTIMATE VEGAUltimateScorer class
    
    两项关键修复:
    1. E^V = exp(-D_B) 转换为相似度 (修复 Pearson 负相关问题)
    2. s_n = mean(cos_sim) 直接使用原始相似度 (修复维度惩罚问题)
    3. s = s_n + s_e 自然融合
    """
    print_detail("Computing VEGA score (终极版本 - VEGAUltimateScorer):", indent=4)
    ...
    try:
        # Use ULTIMATE VEGAUltimateScorer class
        ...
        print_detail("  - Node similarity: %.4f (直接 Cosine 相似度)" % node_sim, indent=6)
        print_detail("  - Edge similarity: %.4f (Bhattacharyya 系数)" % edge_sim, indent=6)
        print_detail("  - Pearson correlation: %.4f (应该为正)" % pearson_corr, indent=6)
...
def run_single_dataset_benchmark(
    ...
    for idx, model_name in enumerate(model_list):
        ...
        cached_vega = load_cache(model_name, dataset_name, 'vega', pca_dim)
        ...
        if cached_vega is not None:
            ...
            vega_score, vega_detail = compute_vega_score_ultimate(
                img_feat, text_feat, logits, model_name,
                pca_dim=pca_dim
            )
            if vega_score is not None:
                save_cache(model_name, dataset_name, 'vega', vega_detail, pca_dim)
        if vega_score is not None:
    ...
    print("\n" + "-" * 70)
    print("VEGA Detailed Analysis (修复版):")
    print("-" * 70)
...
def print_final_results(all_results, pca_dim=64):
    """Print final summary results"""
    print("\n" + "=" * 70)
    print("Summary Results (终极版本 - VEGAUltimateScorer)")
    print("=" * 70)
    print("PCA dim: %d" % pca_dim)
    print("\n关键修复:")
    print("  1. E^V = exp(-D_B) 转换为相似度 (修复 Pearson 负相关)")
    print("  2. s_n = mean(cos_sim) 直接使用原始相似度 (修复维度惩罚)")
    ...
    if valid_vega:
        ...
        print("\nVEGA-Ultimate (on %d datasets):" % len(valid_vega))
        ...
        print("\nVEGA-Ultimate: No valid results")
...
def main():
    ...
    print("=" * 70)
    print("VEGA vs LogME Benchmark (终极版本 - VEGAUltimateScorer)")
    print("=" * 70)
    print("Data directory: %s" % data_dir)
    print("Cache directory: %s" % CACHE_DIR)
    print("PCA dimension: %d" % pca_dim)
    print("\n此版本使用 VEGAUltimateScorer，包含以下修复:")
    print("  1. 【修复 s_e】E^V = exp(-D_B) 转换为相似度")
    print("     - 解决 Pearson(similarity, distance) 负相关问题")
    print("  2. 【修复 s_n】直接使用原始 Cosine 相似度")
    print("     - 删除 Softmax，解决维度惩罚问题")
    print("  3. 【自然融合】s = s_n + s_e")
    print("     - 无需权重 hack")
    ...
    print_final_results(all_results, pca_dim)
    
    result_file = CACHE_DIR / "benchmark_results_ultimate.json"
